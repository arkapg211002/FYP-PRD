...
\begin{tcolorbox}[colback=gray!5!white, colframe=gray!80!black, boxrule=0.5pt, title=Applying Ensemble Model] 
    \begin{lstlisting}[language=Python]
...
\noindent
The above code implements an ensemble learning strategy to classify mental health issues using a combination of Logistic Regression (LR) and XGBoost models. First, the input text is preprocessed separately for each model using respective vectorizers (\texttt{lr\_vectorizer} for LR and \texttt{tfidf\_vectorizer} for XGBoost). The predicted probabilities (\texttt{predict\_proba}) from both models are obtained and concatenated to form a combined feature vector (\texttt{stacked\_features}). This vector serves as input to a meta-learner, which uses these combined probabilities to make the final classification (\texttt{final\_prediction}) and compute the confidence level \\(\texttt{final\_prediction\_proba}). The result is then decoded using a label encoder to determine the most likely mental health issue and its corresponding probability. This approach ensures robust predictions by leveraging the strengths of both base models and improves classification accuracy by combining their individual outputs in a unified manner.

\begin{tcolorbox}[colback=gray!5!white, colframe=gray!80!black, boxrule=0.5pt, title=Retraining the Ensemble Model]
    \begin{lstlisting}[language=Python]
def retrain_model():
    """Retrain the ensemble model with the updated dataset."""
...
\noindent
The above function, \texttt{retrain\_model}, is designed to retrain an ensemble learning model using an updated dataset. Initially, it loads the base models (Logistic Regression and XGBoost), along with their respective vectorizers and label encoder, from previously saved pickle files. After loading the updated dataset, it checks for required columns, preprocesses the data, and splits it into features (\texttt{X\_test}) and target labels (\texttt{y\_test}). The target labels are encoded using the label encoder, while the text features are transformed using the corresponding vectorizers. Predictions from the base models are then combined into a single feature set (\texttt{stacked\_features}) to be used for retraining the meta-learner. The meta-learner is a Logistic Regression model that learns to combine base model predictions effectively. Once trained, the meta-learner is saved for future use, and its performance is evaluated using accuracy. The function returns the trained meta-learner and its accuracy, ensuring that the ensemble model adapts dynamically to any updates in the dataset.
